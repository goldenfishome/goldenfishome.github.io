...
all_rules <- arules::apriori(myData,parameter = list(support=.1,
                                                     confidence=0.5,
                                                     minlen=2,
                                                     maxlen=2))
...
inspect(all_rules)
##  SOrt by Conf
SortedRules_conf <- sort(all_rules, by="confidence", decreasing=TRUE)
inspect(SortedRules_conf[1:20])
## Sort by Sup
SortedRules_sup <- sort(all_rules, by="support", decreasing=TRUE)
inspect(SortedRules_sup[1:20])
#inspect(SortedRules_sup)
## Sort by Lift
SortedRules_lift <- sort(all_rules, by="lift", decreasing=TRUE)
inspect(SortedRules_lift[1:20])

tr.rules <-SortedRules_lift[1:100]
...
D3_network_records <- networkD3::forceNetwork(
  ...
  linkDistance = networkD3::JS("function(d) { return d.value*230; }"), # Function to determine distance between any two nodes, uses variables already defined in forceNetwork function (not variables from a data frame)
...
networkD3::saveNetwork(D3_network_records, 
                       "NetD3_allData.html", selfcontained = TRUE)
...
visNetwork(lenNode, lenEdge) %>% 
  ...
  visEdges(arrows = "middle") %>% visNodes(font = list(size=30))
